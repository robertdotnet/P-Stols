...
public class Player : MonoBehaviour
{
    ...
    private void StartingPosition(GameObject missileObject)
    {
        Vector3 positionAdjustment = new Vector3(-2, 0, -2);
        Vector3 positionAdjuster = this.transform.position + positionAdjustment;
        missileObject.transform.position = positionAdjuster;
        Vector3 rotationAdjustment = this.transform.rotation.eulerAngles;
        rotationAdjustment.y += 180;
        missileObject.transform.eulerAngles = rotationAdjustment; //this.transform.eulerAngles.y + 180;   // + adjuster;
    }
